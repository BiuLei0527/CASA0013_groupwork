...
count_msoa['Count'] = PowerTransformer().fit_transform(count_msoa['Count'].to_numpy().reshape(-1, 1))
price_msoa = stl_msoa.groupby(by='MSOA11CD').price.agg('median')
...
reviews_msoa = stl_msoa.groupby(by='MSOA11CD')["number_of_reviews"].agg('mean')
...
scores = []
for k in range(2,32):
    kmeans = KMeans(n_clusters=k, n_init=25, random_state=42).fit(msoa_pca)
    ave_score = silhouette_score(msoa_pca, kmeans.labels_)
    k_range.append(k)
    scores.append(ave_score)
plt.plot(k_range, scores)
plt.gcf().suptitle("Average Silhouette Scores");
