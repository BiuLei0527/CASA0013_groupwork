...
```{python}
# Distribution of listings by boroughs
...
```{python}
#plot distribution of price below 2000
price=df[df['price']<=2000]['price'].dropna()
f,ax = plt.subplots(figsize=(8, 6))
ax.hist(price, bins=30, edgecolor="black", color="red")
ax.set_xlabel("Price")
ax.set_ylabel("Frequency")
...
bnb_stl = bnb_gdf[bnb_gdf['maximum_nights']<=90][['price', 'room_type', 'property_type', 'maximum_nights', 'geometry']]
...
```{python}
stl_ptype = stl['room_type'].value_counts()
stl_ptype.head()
```
