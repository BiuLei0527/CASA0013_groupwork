...
sorted1 = property.sort_values(ascending=False).reset_index()
sorted_min = sorted1["counts"].min()
...
```{python}
verfications_h = df1['host_verifications'].dropna().reset_index()
print(f"Most hosts use {verifications_h.loc[verifications_h['count'].idxmax(), 'host_verifications']} as methods of verifications as high as {verifications_h['count'].max()}, least hosts use {verifications_h.loc[verifications_h['count'].idxmin(), 'host_verifications']} as ways of verification as low as {verifications_h['count'].min()}")
```
...
print(f"there are {host_filtered.shape[0]} whose listings are more than 100")
```
