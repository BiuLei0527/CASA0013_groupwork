...
boroughs=gdf.read_file(file_name)
```

```{python}
bnb_gdf = gdf_bnb.to_crs(boroughs.crs)
fig, ax=plt.subplots(figsize=(12, 8))
boroughs.plot(ax=ax, facecolor='grey', edgecolor='black', alpha=0.7)
types=bnb_gdf['room_type'].unique().tolist()
colors=['red','green','skyblue','purple']
for t in types:
    subset=bnb_gdf[bnb_gdf['room_type']==t]
    subset.plot(ax=ax, markersize=1, color=colors[types.index(t)], label=t)
...
sorted = property.sort_values(ascending=False)
print(sorted)
```
